...
namespace REL
{
	...
	{
		return true;
		std::string fileName = "Data/SKSE/Plugins/version-se.bin";
		...
		_MESSAGE("Path is %s", fileName.c_str());
...
//		ss.open("out.txt", std::ios::out);
		for (std::size_t i = 0; i < _header.AddrCount(); ++i) {
			...
			a_buf[id] = offset;
//			ss << id << " : " << std::hex << offset << std::dec << std::endl;
#ifdef _DEBUG
			_ids[offset] = id;
#endif
