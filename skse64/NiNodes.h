...
public:
	...
	NiTArray <NiAVObject *>	m_children;	// 138

	static NiNode * Create(UInt32 arrBufLen = 0);

	DEFINE_MEMBER_FN_1(ctor, NiNode*, 0x00C9CCD0, UInt16 arrBufLen);
};
STATIC_ASSERT(offsetof(NiNode, m_children) == 0x138);
...
{
public:
	UInt64	unk150[(0x180 - 0x150) >> 3];

	static BSFadeNode * Create();

	MEMBER_FN_PREFIX(BSFadeNode);
	DEFINE_MEMBER_FN(ctor, BSFadeNode *, 0x012C8230);
};
